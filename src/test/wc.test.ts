...
*/
const WOODateMinus = async(data: Partial<{ years: any; month: any; days: any; hours: any; minutes: any; }>)=>{
    const dateTime = DateTime.now()
    const newDate = dateTime.minus({ years: data.years || 0, months:data.month || 0, days: data.days || 0 , hours:data.hours || 0, minutes:data.minutes || 0}).startOf('day').setZone('America/Fortaleza');
...
* @returns date from time, day, month or years ago in Iso Format
*/
const WOODatePlus = async(data: Partial<{ years: any; month: any; days: any; hours: any; minutes: any; }>)=>{
    const dateTime = DateTime.now()
    const newDate = dateTime.plus({ years: data.years || 0, months:data.month || 0, days: data.days || 0 , hours:data.hours || 0, minutes:data.minutes || 0}).startOf('day').setZone('America/Fortaleza');
    return newDate.toISO()
}
...
describe.only("#options/#methods", () => {
    ...
    test("_normalizeQueryString should return query string sorted by name", () => {
        const url = env.url + "/wp-json/wc/v3/" + endpoint + "?filter[q]=Woo+Album&fields=id&filter[limit]=1";
        const expected = env.url + "/wp-json/wc/v3/" + endpoint + "?fields=id&filter[limit]=1&filter[q]=Woo%20Album";
...
describe.only("Test Coupons", () => {
    ...
    beforeEach(() => {
        ...
        first_name = `${first_name.charAt(0).toLocaleUpperCase()}${first_name.slice(1).toLocaleLowerCase()}` || " ";
        middle_name = String(each.billing?.first_name).split(" ")[1] || " ";
        middle_name = `${middle_name.charAt(0).toLocaleUpperCase()}${middle_name.slice(1).toLocaleLowerCase()}` || " ";
        last_name= `${String(each.billing?.last_name).charAt(0).toLocaleUpperCase()}${String(each.billing?.last_name).slice(1).toLocaleLowerCase()}` || " ";
    ...
    test("should create a Coupon", async () => {
        ...
        const total_products_price_with_discount = (total_products_price * (Number( discount ) / 100)).toFixed(2);
        ...
        const coupom_data:CouponsParams = {
            code: (first_name.substring(0,3)).toLocaleUpperCase() + randomstring.generate({length:2, capitalization:"uppercase", charset: "numeric"}) + `-${each.id}`,
        ...
        if (coupons.data.length > 0) {
            ...
            console.table([{ Keys: expectedKeys.length, Expected: keys.length, diff: expectedKeys.length - keys.length, couponID: coupons.data.id}]);
    ...
    test("should retrive a Coupon", async () => {
        ...
        const coupons = await wooCommerce.get("coupons", {code: getAllCoupons.data[0].id})
    ...
    test("should update a Coupon", async () => {
        ...
        const coupons = await wooCommerce.put("coupons", 
            {
                description: "Campanha de CashBack para clientes Test - Updated - " + randomstring.generate({length:4, capitalization:"uppercase", charset: "alphanumeric"})
            },
            {
    ...
    test("should delete a Coupon", async () => {
        ...
        const coupons = await wooCommerce.delete("coupons", {force:true}, {id: getAllCoupons.data[0].id})
        console.log(coupons.data);
        expect(coupons).toBeInstanceOf(Object);
...
describe.only("Test Products", () => {
    ...
    test("should retrive a product", async () => {
        ...
        const products = await wooCommerce.get("products", {id: getAllProducts.data[0].id})
        // console.log(products.data);
    ...
    test("should update a product", async () => {
        ...
        const products = await wooCommerce.put("products", data, {id: getAllProducts.data[0].id});
        expect(products).toBeInstanceOf(Object);
        if (products.headers["x-wp-totalpages"] > 1) {
            expect(products).toHaveProperty("data", expect.any(Array));
        }
    ...
    test("should delete a product", async () => {
        ...
        const products = await wooCommerce.delete("products", {force: true}, {id: getAllProducts.data[0].id});
        expect(products).toBeInstanceOf(Object);
        if (products.headers["x-wp-totalpages"] > 1) {
            expect(products).toHaveProperty("data", expect.any(Array));
        }
...
describe.only("Test Orders", () => {
    ...
    test("should get an order", async () => {
        ...
        const orders = await wooCommerce.get("orders", {id: getAllOrders.data[0].id});
    ...
    test("should delete an order", async () => {
        ...
        const order = await wooCommerce.delete("orders", {force: true}, {id: getAllCoupons.data[0].id});
        console.log("Order", order.data);
        expect(order).toBeInstanceOf(Object);
        if (order.headers["x-wp-totalpages"] > 1) {
            expect(order).toHaveProperty("data", expect.any(Array));
...
describe("Test Customers", () => {
    ...
    test.only("should get a customer", async () => {
        ...
        const customer = await wooCommerce.get("customers", {id: getAllCustomers.data[0].id});
    ...
    test("should update a customer", async () => {
        ...
        const customer = await wooCommerce.put("customers", {id: getAllCustomers.data[0].id, data});
        console.log("Customer", customer.data);
        expect(customer).toBeInstanceOf(Object);
        if (customer.headers["x-wp-totalpages"] > 1) {
            expect(customer).toHaveProperty("data", expect.any(Array));
    ...
    test.only("should delete a customer", async () => {
        ...
        const customer = await wooCommerce.delete("customers",{force: true}, {id: getAllCustomers.data[0].id});
        console.log("Customer", customer.data);
        expect(customer).toBeInstanceOf(Object);
        if (customer.headers["x-wp-totalpages"] > 1) {
            expect(customer).toHaveProperty("data", expect.any(Array));
